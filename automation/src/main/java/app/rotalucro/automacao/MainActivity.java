...
public class MainActivity extends Activity {
    ...
    private void showHome() {
        ...
        box.addView(text("ROTA LUCRO - AUTOMAÇÃO", 25, 0xFFA8FF38, true));
        TextView sub = text("Módulo separado de análise automática", 15, Color.WHITE, true);
        ...
        Button open = button("Abrir Rota Lucro principal", 0xFF58D215);
        open.setOnClickListener(v -> openMainApp());
        box.addView(open);
        ...
        TextView note = text("Este aplicativo é separado do Rota Lucro principal. Somente este módulo possui Serviço de Acessibilidade para ler ofertas da Uber Driver e 99 Motorista.", 12, 0xFF8FA6B0, false);
    ...
    private void openMainApp() {
        Intent launch = getPackageManager().getLaunchIntentForPackage("app.rotalucro.motorista");
        if (launch != null) {
            startActivity(launch);
        } else {
            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://rota-lucro-pwa-v61-botao-confirmar.vercel.app/")));
        }
    }
    ...
    private void showDisclosure() {
        ...
        TextView disclosure = text(
            ...
            "Este módulo separado precisa ler o texto visível das ofertas na Uber Driver ou 99 Motorista.\n\n" +
            ...
            "O módulo não toca nos botões da Uber/99, não aceita nem recusa corridas automaticamente e não envia o conteúdo lido para um servidor.",
            14, 0xFFD9E6EB, false);
        ...
        agree.setOnCheckedChangeListener((b, checked) -> {
            consent.setEnabled(checked);
            consent.setAlpha(checked ? 1f : .45f);
        });
    ...
    private EditText numeric(String hint, float value) {
        ...
        e.setBackgroundColor(0xFF071A24);
